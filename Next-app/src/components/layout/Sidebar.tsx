...
import { NAV_ITEMS, APP_NAME_URDU } from "@/lib/constants";
import {
  LayoutDashboard,
  Brain,
  CalendarDays,
...
export function Sidebar({ className }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "flex h-full w-64 flex-col border-l border-border bg-surface",
        className
      )}
    >
      {/* Logo */}
      <div className="flex items-center gap-3 px-6 py-5 border-b border-border">
        ...
        <span className="text-xl font-bold text-text">{APP_NAME_URDU}</span>
      </div>
